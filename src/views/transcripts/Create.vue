<template>
  <form @submit.prevent="sendAudio" class="max-w-lg mx-auto">
    ...
    <div class="grid  gap-4">
      ...
      <FormGroup light-mode label="Name" placeholder="A great podcast name" type="text"  required v-model="audio.name"/>
      <Btn type="submit" class="justify-self-start mt-3" :class="{'btn-loading': isLoading}" >Submit</Btn>
    </div>
  </form>
</template>
...
const sendAudio = async () => {
  ...
  try {
    ...
    const file = await storage.createFile("635425752912fbd16b02", "unique()", audio.file)
    ...
    await router.push({name: ROUTE_SINGLE_TRANSCRIPT, params : {id : data?.data?.$id }})
  } catch (e) {
